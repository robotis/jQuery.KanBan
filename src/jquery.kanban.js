...
;(function($, window, document, undefined){	
	...
	Kanban.prototype = {
			...
			kanban.request(send, function(data) {
				...
				if($.inArray('user', options) > -1 && data.users) {
					...
					$.each(data.users, function(i, user) {
						user.src = kanban.fill_user_img(user);
						userlist.append(kanban.fill_user(user, kanban.config.prefix + 'user', true, true));
					});
				}
		...
		fill_user : function(user, cls, draggable, clickable) {
			var kanban = this;
			var li = cls
			 	? $('<li>', {'class' : cls})
		...
		edit_form : function(task) {
			...
			var ps = $('<select>', {'name': 'priority'});
			$.each(kanban.config.prioritys, function(i, v) {
				ps.append($('<option>', {'value': i, 'text': v.name}));
			});
			form.append(ps);
